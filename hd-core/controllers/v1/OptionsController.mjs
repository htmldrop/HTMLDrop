...
export default (context) => {
  ...
  router.get('/', async (req, res, next) => {
    ...
    const hasAccess = await req.guard.user({ canOneOf: ['read', 'read_option'], userId: req?.user?.id })
  ...
  router.post('/', async (req, res, next) => {
    ...
    const hasAccess = await req.guard.user({ canOneOf: ['create', 'create_options'], userId: req?.user?.id })
    if (!hasAccess) return res.status(403).json({ error: 'Permission denied' })
    ...
    const created = await knex(table('options')).where('id', id).first()
  ...
  router.patch('/:idOrName', async (req, res, next) => {
    ...
    const { idOrName } = req.params
    const option = await knex(table('options')).where('id', idOrName).orWhere('name', idOrName).first()
    if (!option) return res.status(404).json({ error: 'Option not found' })

    const id = option.id

    const hasAccess = await req.guard.user({ canOneOf: ['edit', 'edit_options'], userId: req?.user?.id })
    if (!hasAccess) return res.status(403).json({ error: 'Permission denied' })
    ...
    const updated = await knex(table('options')).where('id', id).first()
  ...
  router.delete('/:idOrName', async (req, res, next) => {
    ...
    const hasAccess = await req.guard.user({ canOneOf: ['delete', 'delete_options'], userId: req?.user?.id })
    if (!hasAccess) return res.status(403).json({ error: 'Permission denied' })
