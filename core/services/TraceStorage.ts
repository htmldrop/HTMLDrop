...
export default class TraceStorage {
   ...
   * Get recent traces
   */
  getRecent(options: GetRecentOptions = {}): Trace[] {
    ...
    if (minDuration) {
      filtered = filtered.filter((t) => (t.metadata?.totalDuration || 0) >= minDuration)
    }

    if (errorsOnly) {
      filtered = filtered.filter((t) => (t.summary?.errorCount || 0) > 0)
    }
   ...
   */
  getStats(options: GetStatsOptions = {}): Record<string, any> {
    const { timeWindowMs = 300000 } = options
    const now = Date.now()
    const cutoff = now - timeWindowMs

    const recentTraces = this.traces.filter((t) => (t.storedAt || 0) >= cutoff)
    ...
    // Aggregate by path
    const byPath: Record<string, { count: number; totalDuration: number; avgDuration: number }> = {}
    for (const trace of recentTraces) {
    ...
    // Error rate
    const errorCount = recentTraces.filter((t) => (t.summary?.errorCount || 0) > 0).length
   ...
   */
  findBottlenecks(options: BottleneckOptions = {}): Bottleneck[] {
    ...
    const spanStats = new Map<string, {
      name: string
      category: string
      count: number
      totalDuration: number
  ...
  cleanup(): void {
    ...
    while (this.traces.length > 0 && (this.traces[0].storedAt || 0) < cutoff) {
      const evicted = this.traces.shift()
      if (evicted) {
        this.traceMap.delete(evicted.traceId)
        this.stats.totalEvicted++
