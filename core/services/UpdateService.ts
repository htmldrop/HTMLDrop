...
// @ts-ignore - semver has no type declarations
...
class UpdateService {
   ...
   */
  async getLatestCommit(): Promise<VersionInfo> {
    try {
      const { owner, repo } = await this.getRepositoryInfo()
      ...
      try {
        ...
        if (packageResponse.ok) {
          const packageData = await packageResponse.json() as { content: string }
   ...
   */
  async getLocalCommitSHA(): Promise<string> {
    try {
   ...
   */
  async pullUpdate(options: { branch?: string } = {}): Promise<{ success: boolean; version: string; message: string }> {
    ...
    try {
      ...
      await git.fetch({
        ...
        depth: 1,
        onProgress: (progress) => {
          const percentage = Math.floor((progress.loaded / (progress.total || 1)) * 30) + 20
      ...
      try {
        const BadgeCountServiceModule = await import('./BadgeCountService.ts')
        const BadgeCountService = BadgeCountServiceModule.default
   ...
   */
  private async _runCommand(command: string, args: string[], errorMessage: string): Promise<void> {
   ...
   */
  async restartServer(options: { fullRestart?: boolean } = {}): Promise<void> {
