...
interface TraceStorageDBOptions {
...
export default class TraceStorageDB {
  ...
  private traceMap: Map<string, Trace> = new Map()
  ...
  constructor(options: TraceStorageDBOptions) {
   ...
   * Store a trace
   */
  async store(trace: Trace): Promise<void> {
   ...
   */
  private _addToMemoryCache(trace: Trace): void {
  ...
  async get(traceId: string): Promise<Trace | null> {
    ...
    try {
      const row = await knex(table('traces')).where('trace_id', traceId).first() as TraceRow | undefined
      if (!row) return null
   ...
   */
  private _rowToTrace(row: TraceRow): Trace {
    return {
      traceId: row.trace_id,
      request: {
        method: row.request_method || undefined,
        path: row.request_path || undefined,
        url: row.request_url || undefined,
   ...
   */
  async getRecent(options: GetRecentOptions = {}): Promise<Trace[]> {
    ...
    try {
      ...
      const rows = await query as TraceRow[]
   ...
   * Get slowest traces
   */
  async getSlowest(limit = 10, hoursBack = 24): Promise<Trace[]> {
    const { knex, table } = this.context
    if (!knex) {
   ...
   */
  async getErrors(limit = 20, hoursBack = 24): Promise<Trace[]> {
    const { knex, table } = this.context
    if (!knex) {
      return [...this.recentTraces]
        .filter((t) => (t.summary?.errorCount || 0) > 0)
        .reverse()
        .slice(0, limit)
    }
   ...
   */
  async getStats(options: GetStatsOptions = {}): Promise<Record<string, any>> {
    ...
    try {
      ...
      const durations = await knex(table('traces'))
        ...
        .orderBy('total_duration', 'asc') as { total_duration: number }[]
      ...
      const byPathMap: Record<string, { count: number; totalDuration: number; avgDuration: number }> = {}
   ...
   */
  private _getMemoryStats(timeWindowMs: number): Record<string, any> {
    const now = Date.now()
    const cutoff = now - timeWindowMs
    const recent = this.recentTraces.filter((t) => (t.storedAt || 0) >= cutoff)
    ...
    return {
      ...
      errorRate: `${((recent.filter((t) => (t.summary?.errorCount || 0) > 0).length / recent.length) * 100).toFixed(2)}%`,
      byPath: {},
      storage: {
        memoryCacheSize: this.recentTraces.length
      }
   ...
   */
  private async _archiveTraces(cutoff: Date): Promise<void> {
    const { knex, table } = this.context
    if (!knex) return
    ...
    try {
      ...
      const traces = await knex(table('traces'))
        ...
        .where('created_at', '<', cutoff) as TraceRow[]
  ...
  async count(): Promise<number> {
    ...
    try {
      const result = await knex(table('traces')).count('* as count').first() as { count: number } | undefined
   ...
   */
  async toJSON(limit = 1000): Promise<string> {
