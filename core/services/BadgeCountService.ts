...
const fetchPackageVersion = async (packageName: string, timeout = 5000): Promise<string | null> => {
...
const processBatch = async <T>(items: Promise<T>[], batchSize: number): Promise<T[]> => {
  const results: T[] = []
...
const countPluginUpdates = async (): Promise<number> => {
  try {
    ...
    // Prepare check tasks
    const checkTasks: Promise<PackageVersionResult>[] = []
      ...
      try {
        ...
        checkTasks.push(
          fetchPackageVersion(packageJson.name).then(latestVersion => ({
            ...
            hasUpdate: latestVersion !== null && latestVersion !== packageJson.version
          }))
        )
      } catch (err) {
...
const countThemeUpdates = async (): Promise<number> => {
  try {
...
export default class BadgeCountService {
  ...
  private knex: NonNullable<HTMLDrop.Context['knex']>
  private table: (name: string) => string

  constructor(context: HTMLDrop.Context) {
    if (!context.knex) {
      throw new Error('BadgeCountService requires a database connection')
    }
   ...
   * @param force - If true, bypass cache TTL and force update
   */
  async updateBadgeCounts(force = false): Promise<{
    updated: boolean
    message?: string
    pluginCount?: number
    themeCount?: number
   ...
   */
  async getBadgeCounts(): Promise<{ plugins: number; themes: number; cms: number }> {
    try {
      const [pluginCache, themeCache, cmsCache] = await Promise.all([
        this.knex(this.table('options')).where('name', 'badge_count_plugins_cache').first(),
        this.knex(this.table('options')).where('name', 'badge_count_themes_cache').first(),
   ...
   */
  async _checkCmsUpdate(): Promise<number> {
    try {
   ...
   * @param type - Cache type: 'plugins', 'themes', or 'cms'
   */
  async clearCache(type: 'plugins' | 'themes' | 'cms'): Promise<void> {
    try {
