...
interface ProviderRequest {
  req: Request & {
    ...
    context: {
      ...
      parseVue: (filePath: string) => Promise<string>
      knex: Knex
...
const getBadgeCounts = async ({ knex, table }: { knex: Knex; table: (name: string) => string }): Promise<{
  pluginsBadge: number
  themesBadge: number
  cmsBadge: number
}> => {
