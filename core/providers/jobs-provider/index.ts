...
export default async function JobsProvider({ req }: ProviderRequest): Promise<void> {
  const { hooks } = req

  // Register jobs post type
  hooks.addAction('init', async () => {
    const postType: PostTypeDefinition = {
      name: 'Jobs',
      slug: 'jobs',
    ...
    await hooks.registerPostType(postType)
  })
}
