<template>
  ...
  <div class="container" v-else>
    <div class="header-section">
      ...
      <router-link class="add-button" :to="'/' + slug + '/terms/' + taxonomy + '/new'">{{ translate('Add') }} {{ translate(taxonomyItem?.name_singular)?.toLowerCase() }}</router-link>
      <teleport to="body">
        <component
          v-if="mediaUploader"
    ...
    <div class="tabs-and-search">
      ...
      <div class="search">
        ...
        <button @click="offset = 0; getItems()" class="search-button">{{ translate('Search in') }} {{ translate(taxonomyItem?.name_plural)?.toLowerCase() }}</button>
      </div>
    </div>
    ...
    <div class="table-wrapper">
      <div class="table">
        <div class="header">
          ...
          <div v-for="{ field, priority, source } in taxonomyFields || []" :key="field.slug" class="field">
        ...
        <div class="body">
          <div v-for="item in response?.items || []" :key="item.id" class="row">
            ...
            <div class="actions-cell sticky-left">
              <router-link class="button" :to="'/' + slug + '/terms/' + taxonomy + '/' + (taxonomyFields?.some(f => f.field?.slug === 'slug') ? item.slug : item.id)">{{ translate('Open') }}</router-link>
...
export default {
  ...
  methods: {
    ...
    async create(item) {
      await this.apiFetch(`${this.apiBase}/api/v1/${this.slug}/terms/${this.taxonomy}`, {
        method: 'POST',
    ...
    async save(item) {
      ...
      const result = await this.apiFetch(`${this.apiBase}/api/v1/${this.slug}/terms/${this.taxonomy}/${item.id}`, {
        method: 'PATCH',
    ...
    async trash(item, permanently = false, quiet = false) {
      ...
      await this.apiFetch(`${this.apiBase}/api/v1/${this.slug}/terms/${this.taxonomy}/${item.id}${permanently ? '?permanently=true' : ''}`, {
        method: 'DELETE'
      })
    ...
    async restore(item) {
      const originalItem = this.responseOriginal?.items.find(itm => itm.id === item.id)
      const status = originalItem.status
      await this.apiFetch(`${this.apiBase}/api/v1/${this.slug}/terms/${this.taxonomy}/${item.id}`, {
        method: 'PATCH',
    ...
    async init() {
      ...
      this.selectedItems = []
      await Promise.all([this.getPostType(), this.getPostTypeFields(), this.getTaxonomy(), this.getTaxonomyFields(), this.getItems()])
      this.selectedItems = []
      this.allSelected = false
    },
    async getPostType() {
    ...
    async getItems() {
      ...
      if (this.parent_id) params.set('parent_id', this.parent_id)
      ...
      const url = `${this.apiBase}/api/v1/${this.slug}/terms/${this.taxonomy}${params.toString() ? '?' + params.toString() : ''}`
