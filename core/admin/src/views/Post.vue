  ...
  <div style="padding: 20px 20px 50px" v-else>
    ...
    <div class="wrapper">
      <div class="content">
        ...
        <div>
          <router-link class="button" :to="'/' + slug">
        ...
        <div class="grid">
          <card>
            ...
            <div class="fields">
              <div v-for="{ field, priority, source } in postTypeFields?.filter(f => !['slug', 'title', 'featured_image', 'status'].includes(f.field.slug))" class="field">
      ...
      <div class="grid">
        ...
        <template v-for="{ name_plural, slug } in postTypeTaxonomies || []" :key="'taxonomy_' + slug">
          <card v-if="obj.terms && obj.terms[slug] !== undefined">
            <template #header>
              <h2>{{ translate(name_plural) }}</h2>
            </template>
            ...
            <div style="display: flex;flex-direction: column;">
              ...
              <input
                type="text"
                v-model="termSearch[slug]"
                @input="searchTermsDebounced(slug, termSearch[slug])"
                :placeholder="'Search ' + slug"
              />

              <!-- Multi-select with filtered terms -->
              <select multiple v-model="obj.terms[slug]" style="height: 100px">
                <option
                  v-for="term in termResults[slug] || []"
                  :key="term.id"
                  :value="term"
                >
...
export default {
  ...
  computed: {
    ...
    featuredImage() {
      return this.postTypeFields?.find(f => f.field.slug === 'featured_image' && f.field.type === 'media')
    }
  },
  watch: {
    navTree(newVal) {
  ...
  methods: {
    getTermsOptions(item, slug) {
      return this.termResults?.[slug] || []
    },
    ...
    async searchTerms(slug, query) {
      const currentTerms = this.obj?.terms?.[slug] || []

      if (!query) {
        // Only show currently attached terms
        this.termResults[slug] = [...currentTerms]
        return
      }
    ...
    async init() {
      ...
      // Initialize all taxonomy term arrays before getPost
      for (const { slug } of this.postTypeTaxonomies || []) {
        this.obj.terms[slug] = []
        this.termResults[slug] = []
      }

      await this.getPost()

      // Reset slug for new posts
      if (this.isCreating) {
        this.newSlug = ''
      }
    },
    ...
    asign(obj) {
      ...
      // Preserve obj structure and initialize terms
      const newObj = Object.fromEntries(Object.entries(obj))
      if (!newObj.terms) newObj.terms = {}

      // Initialize all taxonomy term arrays BEFORE assigning to this.obj
      for (const { slug } of this.postTypeTaxonomies || []) {
        if (!newObj.terms[slug]) newObj.terms[slug] = []
      }

      // Now assign the fully initialized object
      this.obj = newObj

      // Initialize termResults with current terms
      for (const { slug } of this.postTypeTaxonomies || []) {
        this.termResults[slug] = [...this.obj.terms[slug]]
      }
    }
  }
}
