...
router.get('/traces', async (req, res) => {
  try {
    const { limit = 20, offset = 0, path, minDuration, errorsOnly } = req.query
    const traceStorage = req.context.traceStorage
...
router.get('/slowest', async (req, res) => {
  try {
    const { limit = 10, hoursBack = 24 } = req.query
    const traceStorage = req.context.traceStorage
...
router.get('/stats', async (req, res) => {
  try {
    const { timeWindow = 300000 } = req.query
    const traceStorage = req.context.traceStorage
...
router.get('/bottlenecks', async (req, res) => {
  try {
    const { limit = 10, timeWindow = 300000 } = req.query
    const traceStorage = req.context.traceStorage
 ...
 */
router.get('/waterfall/:traceId', async (req, res) => {
  try {
    const { traceId } = req.params
    const traceStorage = req.context.traceStorage
