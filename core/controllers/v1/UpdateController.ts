...
export default (context: HTMLDrop.Context): Router => {
  ...
  router.get('/check', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const updateStatus = await updateService.checkForUpdates() as UpdateStatus

      // Clear badge count cache to ensure it refreshes with latest update status
      try {
        const badgeCountService = new BadgeCountService(guardReq.context)
  ...
  router.get('/current', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const currentVersion: string = await updateService.getCurrentVersion()
      const commitSHA: string = await updateService.getLocalCommitSHA()
  ...
  router.post('/pull', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const { branch = 'main' } = guardReq.body

      const result = await updateService.pullUpdate({ branch }) as PullResult
  ...
  router.get('/status', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const [updateStatus, commitSHA] = await Promise.all([
        updateService.checkForUpdates() as Promise<UpdateStatus>,
        updateService.getLocalCommitSHA() as Promise<string>
      ])
