  ...
  router.post('/', express.json(), async (req, res) => {
    ...
    try {
      ...
      for (const manKey in manualUpdates) {
        const value = manualUpdates[manKey]
      ...
      for (const line of lines) {
        ...
        for (const reqKey in inputVarMapping) {
          if (updates.hasOwnProperty(reqKey) && key === inputVarMapping[reqKey]) {
            let newValue = updates[reqKey]
      ...
      for (const reqKey in inputVarMapping) {
        ...
        if (updates.hasOwnProperty(reqKey) && !updatedKeys.has(envKey)) {
          let newValue = updates[reqKey]
          if (typeof newValue === 'object') {
            newValue = JSON.stringify(newValue)
          }
