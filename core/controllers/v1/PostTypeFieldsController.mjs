...
export default (context) => {
   ...
   */
  const checkCapability = async (req, postTypeSlug, routeCaps) => {
  ...
  router.get('/', async (req, res) => {
    const { getFields } = req.hooks
    const { postType } = req.params
    const type = await checkCapability(req, postType, ['read', 'read_post_type', 'read_post'])
  ...
  router.post('/', async (req, res) => {
    ...
    const { postType } = req.params
    const type = await checkCapability(req, postType, ['edit', 'edit_post_types'])
    if (!type) return res.status(403).json({ error: 'Permission denied or post type not accessible' })
  ...
  router.get('/:id', async (req, res) => {
    ...
    const field = await knex(table('post_type_fields')).where('id', id).first()
  ...
  router.patch('/:id', async (req, res) => {
    ...
    const updated = await knex(table('post_type_fields')).where('id', id).first()
