...
2. **User content** (`content/`) - Named volume required (uploads, plugins, themes)
...
services:
  app:
    ...
    volumes:
      ...
      - content:/app/content          # User content (persistent)
...
volumes:
  ...
  content:   # User uploads, plugins, themes
```

**How it works:**
1. `- .:/app` mounts your application code (no persistence needed)
2. `content` volume persists user uploads, installed plugins, and themes
