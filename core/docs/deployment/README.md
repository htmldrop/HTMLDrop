...
# DB_CLIENT=sqlite3
# DB_PATH=./content/config/database.sqlite
...
module.exports = {
  apps: [{
    ...
    watch: false,
    ignore_watch: ['node_modules', 'content/uploads', 'logs'],
  }]
}
```
...
ExecStart=/usr/bin/node /var/www/htmldrop/core/index.mjs
Restart=always
RestartSec=10
...
server {
    ...
    location /uploads/ {
        alias /var/www/htmldrop/content/uploads/;
        expires 1y;
...
```javascript
// core/services/CacheService.mjs
...
BACKUP_DIR="/backups/htmldrop"
DATE=$(date +%Y%m%d_%H%M%S)
SOURCE="/var/www/htmldrop/content"
...
sudo mount -t nfs nfs-server:/exports/uploads /var/www/htmldrop/content/uploads
