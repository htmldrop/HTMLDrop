...
See the `example-job-plugin` in `/content/plugins/example-job-plugin/` for complete examples of:
...
The provider is located at `core/providers/jobs-provider/index.mjs` and registers the jobs post type with all necessary fields on system initialization.
